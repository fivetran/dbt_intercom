...
final as (
  select 
    ...
    {{ dbt.datediff("conversation_enhanced.conversation_created_at", "conversation_part_aggregates.first_admin_response_at", 'second') }} as time_to_first_response_sec,
    round(cast({{ dbt.datediff("conversation_enhanced.conversation_created_at", "conversation_part_aggregates.first_admin_response_at", 'second') }} as numeric) / 60, 2) as time_to_first_response_minutes,
    conversation_part_aggregates.first_human_admin_response_at,
