...
conversation_events as (
  ...
  select distinct
    ...
    first_value(case when part_type = 'close' and author_type = 'admin' then author_id end)
      over (partition by conversation_id {{ fivetran_utils.partition_by_source_relation(package_name='intercom') }} order by case when part_type = 'close' and author_type = 'admin' then 0 else 1 end,
    ...
    first_value(case when part_type = 'close' then author_id end)
      over (partition by conversation_id {{ fivetran_utils.partition_by_source_relation(package_name='intercom') }} order by case when part_type = 'close' then 0 else 1 end,
    ...
    first_value(case when author_type in ('user','lead') then author_id end)
      over (partition by conversation_id {{ fivetran_utils.partition_by_source_relation(package_name='intercom') }} order by case when author_type in ('user','lead') then 0 else 1 end,
    ...
    first_value(case when assigned_to_type = 'team' then assigned_to_id end)
      over (partition by conversation_id {{ fivetran_utils.partition_by_source_relation(package_name='intercom') }} order by case when assigned_to_type = 'team' then 0 else 1 end,
